#ifndef AMC_DYNAMIC_MODEL_H_
#define AMC_DYNAMIC_MODEL_H_
...
namespace amc::dynamic
{
	...
	/// 本文以scara机器人为例，介绍amc的建模以及运动学和动力学计算方法。
	/// \section Scara机器人
	///
	...
	///
	/// \subsection 添加求解器
	/// amc的基础求解器（ amc::dynamic::UniversalSolver ）可以求解任意机构的运动学和动力学，
	/// 但是该求解器需要用户手动管理机构拓扑，为此还提供其他四种求解器：
	/// + 运动学逆解 amc::dynamic::InverseKinematicSolver
	/// + 运动学正解 amc::dynamic::ForwardKinematicSolver
	/// + 动力学逆解 amc::dynamic::InverseDynamicSolver
	/// + 动力学正解 amc::dynamic::ForwardDynamicSolver
	/// 
	...
	/// \subsection 速度反解
	/// 
	/// 速度模型的求解与位置模型相似， amc 提供2种速度表达方式，角速度与线速度(va)，速度螺旋(vs)，分别对应以下函数：
	...
	///
	/// \subsection 用Adams验证
	/// 为了验证计算结果是否正确，可以使用其他软件对结构进行验证，比如Adams。amc提供插件，让用户可以直接保存成Adams模型。
	/// 
	/// 以下代码展示使用Adams验证的过程：
	...
	/// @{
	///
	class Model :public amc::core::Object
	{
	public:
		...
		/// @{
		auto calculator()->amc::core::Calculator&;
		auto calculator()const ->const amc::core::Calculator& { return const_cast<std::decay_t<decltype(*this)> *>(this)->calculator(); }
		auto environment()->amc::dynamic::Environment&;
		auto environment()const ->const amc::dynamic::Environment& { return const_cast<std::decay_t<decltype(*this)> *>(this)->environment(); }
		auto variablePool()->amc::core::ObjectPool<Variable, Element>&;
		auto variablePool()const->const amc::core::ObjectPool<Variable, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->variablePool(); }
		auto partPool()->amc::core::ObjectPool<Part, Element>&;
		auto partPool()const->const amc::core::ObjectPool<Part, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->partPool(); }
		auto jointPool()->amc::core::ObjectPool<Joint, Element>&;
		auto jointPool()const->const amc::core::ObjectPool<Joint, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->jointPool(); }
		auto motionPool()->amc::core::ObjectPool<Motion, Element>&;
		auto motionPool()const->const amc::core::ObjectPool<Motion, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->motionPool(); }
		auto generalMotionPool()->amc::core::ObjectPool<GeneralMotion, Element>&;
		auto generalMotionPool()const->const amc::core::ObjectPool<GeneralMotion, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->generalMotionPool(); }
		auto forcePool()->amc::core::ObjectPool<Force, Element>&;
		auto forcePool()const->const amc::core::ObjectPool<Force, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->forcePool(); }
		auto solverPool()->amc::core::ObjectPool<Solver, Element>&;
		auto solverPool()const->const amc::core::ObjectPool<Solver, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->solverPool(); }
		auto simulatorPool()->amc::core::ObjectPool<Simulator, Element>&;
		auto simulatorPool()const->const amc::core::ObjectPool<Simulator, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->simulatorPool(); }
		auto simResultPool()->amc::core::ObjectPool<SimResult, Element>&;
		auto simResultPool()const->const amc::core::ObjectPool<SimResult, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->simResultPool(); }
		auto calibratorPool()->amc::core::ObjectPool<Calibrator, Element>&;
		auto calibratorPool()const->const amc::core::ObjectPool<Calibrator, Element>& { return const_cast<std::decay_t<decltype(*this)> *>(this)->calibratorPool(); }
		auto ground()->Part&;
		...
		AMC_REGISTER_TYPE(Model);
		AMC_DECLARE_BIG_FOUR(Model);
