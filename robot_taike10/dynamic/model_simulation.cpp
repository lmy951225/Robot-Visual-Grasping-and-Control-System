...
namespace amc::dynamic
{
	...
	auto Calibrator::clbFiles(const std::vector<std::string> &file_paths)->void
	{
		...
		for (auto &file : file_paths)
		{
			...
			auto mtx = amc::dynamic::dlmread(file.c_str());
	...
	{
		auto mtx = amc::dynamic::dlmread(file_paths.c_str());
		...
		for (int i = 0; i<6;++i)
		{
			...
			sprintf(posn, "/home/kk/robot_taike10/test/pos%d.txt", i);
			sprintf(veln, "/home/kk/robot_taike10/test/vel%d.txt", i);
			sprintf(accn, "/home/kk/robot_taike10/test/acc%d.txt", i);
			sprintf(fcen, "/home/kk/robot_taike10/test/fce%d.txt", i);
			sprintf(fn, "/home/kk/robot_taike10/test/f%d.txt", i);
			sprintf(ffn, "/home/kk/robot_taike10/test/ff%d.txt", i);
			sprintf(fdn, "/home/kk/robot_taike10/test/fd%d.txt", i);
	...
	AMC_DEFINE_BIG_FOUR_CPP(Calibrator);

	struct SimResult::TimeResult::Imp { std::deque<double> time_; };
	auto SimResult::TimeResult::saveXml(amc::core::XmlElement &xml_ele)const->void
	{
	...
	auto SimResult::TimeResult::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		// 以下导入数据 //
	...
	auto SimResult::PartResult::saveXml(amc::core::XmlElement &xml_ele)const->void
	{
		Element::saveXml(xml_ele);
	...
	auto SimResult::PartResult::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		// 以下寻找对应的part //
		...
		auto &part_pool = static_cast<amc::core::ObjectPool<Part, Element>&>(*ancestor<Model>()->findByName("part_pool"));
	...
	auto SimResult::ConstraintResult::saveXml(amc::core::XmlElement &xml_ele)const->void
	{
		Element::saveXml(xml_ele);
	...
	auto SimResult::ConstraintResult::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		// 以下寻找对应的constraint //
		...
		{
			auto &pool = static_cast<amc::core::ObjectPool<Joint, Element>&>(*ancestor<Model>()->findByName("joint_pool"));
		...
		{
			auto &pool = static_cast<amc::core::ObjectPool<Motion, Element>&>(*ancestor<Model>()->findByName("motion_pool"));
			auto c = pool.findByName(xml_ele.Attribute("constraint"));
			if (c != pool.end())imp_->constraint_ = &*c;
		}
		if (!imp_->constraint_ && ancestor<Model>()->findByName("general_motion_pool") != ancestor<Model>()->children().end())
		{
			auto &pool = static_cast<amc::core::ObjectPool<GeneralMotion, Element>&>(*ancestor<Model>()->findByName("general_motion_pool"));
			auto c = pool.findByName(xml_ele.Attribute("constraint"));
			if (c != pool.end())imp_->constraint_ = &*c;
		}
	...
	struct SimResult::Imp
	{
		...
		amc::core::ObjectPool<PartResult, Element> *part_result_pool_;
		amc::core::ObjectPool<ConstraintResult, Element> *constraint_result_pool_;
	};
	auto SimResult::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		Element::loadXml(xml_ele);

		imp_->time_result_ = findOrInsertType<TimeResult>();
		imp_->constraint_result_pool_ = findOrInsertType<amc::core::ObjectPool<SimResult::ConstraintResult, Element> >();
		imp_->part_result_pool_ = findOrInsertType<amc::core::ObjectPool<SimResult::PartResult, Element> >();
	}
	auto SimResult::timeResult()->TimeResult& { return *imp_->time_result_; }
	auto SimResult::partResultPool()->amc::core::ObjectPool<SimResult::PartResult, Element>& { return *imp_->part_result_pool_; }
	auto SimResult::constraintResultPool()->amc::core::ObjectPool<SimResult::ConstraintResult, Element>& { return *imp_->constraint_result_pool_; }
	...
	SimResult::SimResult(const std::string &name) : Element(name), imp_(new Imp())
	{
		...
		imp_->part_result_pool_ = &add<amc::core::ObjectPool<SimResult::PartResult, Element> >("part_result_pool");
		imp_->constraint_result_pool_ = &add<amc::core::ObjectPool<SimResult::ConstraintResult, Element> >("constraint_result_pool");
	}
	SimResult::SimResult(const SimResult&other) : Element(other), imp_(other.imp_)
	{
		imp_->time_result_ = findType<TimeResult >("time_result");
		imp_->constraint_result_pool_ = findType<amc::core::ObjectPool<SimResult::ConstraintResult, Element> >("constraint_result_pool");
		imp_->part_result_pool_ = findType<amc::core::ObjectPool<SimResult::PartResult, Element> >("part_result_pool");
	}
	...
	auto Simulator::simulate(amc::plan::Plan &plan, SimResult &result)->void
	{
		result.allocateMemory();
		// 记录初始状态 //
		result.record();

		amc::plan::PlanTarget target
		{
			&plan,
			nullptr,
			&model(),
	...
	AMC_DEFINE_BIG_FOUR_CPP(Simulator);
	...
	auto SolverSimulator::saveXml(amc::core::XmlElement &xml_ele) const->void
	{
		Simulator::saveXml(xml_ele);
		xml_ele.SetAttribute("solver", solver().name().c_str());
	}
	auto SolverSimulator::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		...
		auto &solver_pool = static_cast<amc::core::ObjectPool<Solver, Element>&>(*ancestor<Model>()->findByName("solver_pool"));
	...
	auto SolverSimulator::simulate(amc::plan::Plan &plan, SimResult &result)->void
	{
	...
	AMC_DEFINE_BIG_FOUR_CPP(SolverSimulator);
	...
	AMC_DEFINE_BIG_FOUR_CPP(AdamsSimulator);
}
