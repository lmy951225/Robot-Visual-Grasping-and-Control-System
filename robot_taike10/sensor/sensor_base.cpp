...
namespace amc::sensor
{
	...
	{
		amc::core::ObjectPool<Sensor>* sensor_pool_;
	};
	auto SensorRoot::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		Object::loadXml(xml_ele);
		imp_->sensor_pool_ = findByName("sensor_pool") == children().end() ? &add<amc::core::ObjectPool<Sensor> >("sensor_pool") : static_cast<amc::core::ObjectPool<Sensor> *>(&(*findByName("sensor_pool")));
	}
	auto SensorRoot::sensorPool()->amc::core::ObjectPool<Sensor>& { return *imp_->sensor_pool_; }
	auto SensorRoot::sensorPool()const->const amc::core::ObjectPool<Sensor> & { return *imp_->sensor_pool_; }
	SensorRoot::~SensorRoot() {}
	SensorRoot::SensorRoot(const std::string &name) :Object(name)
	{	
		amc::core::Object::registerTypeGlobal<amc::core::ObjectPool<Sensor> >();
		imp_->sensor_pool_ = &add<amc::core::ObjectPool<Sensor> >("sensor_pool");
	}
	AMC_DEFINE_BIG_FOUR_CPP(SensorRoot);
}
