...
namespace amc::server
{
	const std::uint64_t global_option = 
		amc::plan::Plan::NOT_CHECK_OPERATION_ENABLE | 
		amc::plan::Plan::NOT_CHECK_POS_MAX | 
		amc::plan::Plan::NOT_CHECK_POS_MIN;
	
	struct ControlServer::Imp
	{
		struct InternalData
		{
			std::shared_ptr<amc::plan::PlanTarget> target;
			std::promise<void> ret_promise;
		};
		
		auto tg()->void;
		auto executeCmd(amc::plan::PlanTarget &target)->int;
		...
		amc::dynamic::Model* model_;
		amc::control::Controller* controller_;
		amc::sensor::SensorRoot* sensor_root_;
		amc::plan::PlanRoot* plan_root_;
	...
	auto ControlServer::Imp::tg()->void
	{
		...
		if (cmd_end > cmd_now)
		{
			...
			{
				target.ret_code = amc::plan::PlanTarget::ERROR;
			...
			{
				target.ret_code = amc::plan::PlanTarget::SUCCESS;
				
				if(!(target.option & amc::plan::Plan::NOT_PRINT_EXECUTE_COUNT))
			...
			{
				if (++count_ % 1000 == 0 && !(target.option & amc::plan::Plan::NOT_PRINT_EXECUTE_COUNT))
	...
	auto ControlServer::Imp::executeCmd(amc::plan::PlanTarget &target)->int
	{
		target.count = count_;
		...
		for (std::size_t i = 0; i < std::min(controller_->motionPool().size(), model_->motionPool().size()); ++i)
		{
			...
			if (mm.active())
			{
				if ((target.option & amc::plan::Plan::USE_TARGET_POS))cm.setTargetPos(mm.mp());
				if ((target.option & amc::plan::Plan::USE_TARGET_VEL))cm.setTargetVel(mm.mv());
				if ((target.option & amc::plan::Plan::USE_TARGET_CUR))cm.setTargetCur(mm.mf());
				if ((target.option & amc::plan::Plan::USE_VEL_OFFSET))cm.setOffsetVel(mm.mv());
				if ((target.option & amc::plan::Plan::USE_CUR_OFFSET))cm.setOffsetCur(mm.mf());
			}
		}
	...
	auto ControlServer::Imp::checkMotion(std::uint64_t option)->int
	{
		...
		for (std::size_t i = 0; i < controller_->motionPool().size(); ++i)
		{
			...
			/*
			if (!(option & amc::plan::Plan::NOT_CHECK_OPERATION_ENABLE)
			...
			*/
			...
			if ((cm.statusWord() & 0x6f) == 0x27)
			{
				...
				{
					// check pos max //
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_MAX)
					...
					// check pos min //
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_MIN)
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_CONTINUOUS)
						&& count_ > 1
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_CONTINUOUS_AT_START)
						&& count_ <= 1
						&& ((cm.targetPos() - ld.p) > 0.001 * cm.maxVel() || (cm.targetPos() - ld.p) < 0.001 * cm.minVel()))
					{
						server_->controller().mout() << __FILE__ << __LINE__ << "\n";
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_CONTINUOUS_SECOND_ORDER)
						&& count_ > 2
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_CONTINUOUS_SECOND_ORDER_AT_START)
						&& count_ <= 2
						&& ((cm.targetPos() + lld.p - 2 * ld.p) > 1e-6 * cm.maxAcc() || (cm.targetPos() + lld.p - 2 * ld.p) < 1e-6 * cm.minAcc()))
					{
						server_->controller().mout() << __FILE__ << __LINE__ << "\n";
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_POS_FOLLOWING_ERROR)
				...
				{
					// check vel max //
					if (!(option & amc::plan::Plan::NOT_CHECK_VEL_MAX)
					...
					// check vel min //
					if (!(option & amc::plan::Plan::NOT_CHECK_VEL_MIN)
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_VEL_CONTINUOUS)
						&& count_ > 1
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_VEL_CONTINUOUS_AT_START)
						&& count_ <= 1
						&& ((cm.targetVel() - ld.v) > 0.001 * cm.maxAcc() || (cm.targetVel() - ld.v) < 0.001 * cm.minAcc()))
					{
						server_->controller().mout() << __FILE__ << __LINE__ << "\n";
					...
					if (!(option & amc::plan::Plan::NOT_CHECK_VEL_FOLLOWING_ERROR)
	...
	{
		auto iter = std::find_if(children().begin(), children().end(), [](const amc::core::Object &obj) { return obj.name() == "model"; });
	...
	{
		auto iter = std::find_if(children().begin(), children().end(), [](const amc::core::Object &obj) { return obj.name() == "controller"; });
		if (iter != children().end())children().erase(iter);
		add(controller);
	...
	{
		auto iter = std::find_if(children().begin(), children().end(), [](const amc::core::Object &obj) { return obj.name() == "sensor_root"; });
		if (iter != children().end())children().erase(iter);
		add(sensor_root);
	...
	{
		auto iter = std::find_if(children().begin(), children().end(), [](const amc::core::Object &obj) { return obj.name() == "plan_root"; });
		if (iter != children().end())children().erase(iter);
		add(plan_root);
	...
	auto ControlServer::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		Object::loadXml(xml_ele);
		imp_->controller_ = findOrInsertType<amc::control::Controller>();
		imp_->model_ = findOrInsertType<amc::dynamic::Model>();
		imp_->sensor_root_ = findOrInsertType<amc::sensor::SensorRoot>();
		imp_->plan_root_ = findOrInsertType<amc::plan::PlanRoot>();
		imp_->interface_root_ = findOrInsertType<amc::server::InterfaceRoot>();
	}
	auto ControlServer::executeCmd(const amc::core::Msg &msg)->std::shared_ptr<amc::plan::PlanTarget>
	{
		...
		auto internal_data = std::make_shared<Imp::InternalData>(Imp::InternalData{
			std::make_shared<amc::plan::PlanTarget>(
				amc::plan::PlanTarget
				{ 
					&*plan_iter, 
					this, 
					...
					0, 
					0, 
					amc::control::Master::RtStasticsData{ 0,0,0,0x8fffffff,0,0,0 },
					std::any(),
					amc::plan::PlanTarget::CANCELLED,
					std::future<void>()
				}),
			std::promise<void>()
			});
		...
		// prepair //
		if (!(target->option & amc::plan::Plan::NOT_RUN_PREPAIR_FUNCTION))
		{
			// 等待所有任务完成 //
			if (target->option & amc::plan::Plan::PREPAIR_WHEN_ALL_PLAN_EXECUTED)waitForAllExecution();

			// 等待所有任务收集 //
			if (target->option & amc::plan::Plan::PREPAIR_WHEN_ALL_PLAN_COLLECTED)waitForAllCollection();
		...
		if (!(target->option & amc::plan::Plan::NOT_PRINT_CMD_INFO))std::cout << cmd << std::endl;

		auto &log = LOG_INFO << cmd << std::endl;
		for (auto &p : params)
		{
			if (!(target->option & amc::plan::Plan::NOT_PRINT_CMD_INFO))
				std::cout << std::string(print_size - p.first.length(), ' ') << p.first << " : " << p.second << std::endl;
			if (!(target->option & amc::plan::Plan::NOT_LOG_CMD_INFO))
				log << std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::string(print_size - p.first.length(), ' ') << p.first << " : " << p.second << std::endl;
		}
		if (!(target->option & amc::plan::Plan::NOT_PRINT_CMD_INFO))std::cout << std::endl;
		// print over ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		// execute //
		if (!(target->option & amc::plan::Plan::NOT_RUN_EXECUTE_FUNCTION))
		{
			...
			// 等待所有任务完成 //
			if (target->option & amc::plan::Plan::EXECUTE_WHEN_ALL_PLAN_EXECUTED)waitForAllExecution();

			// 等待所有任务收集 //
			if (target->option & amc::plan::Plan::EXECUTE_WHEN_ALL_PLAN_COLLECTED)waitForAllCollection();

			// 判断是否等待命令池清空 //
			if ((!(target->option & amc::plan::Plan::WAIT_IF_CMD_POOL_IS_FULL)) && (cmd_end - imp_->cmd_collect_.load()) >= Imp::CMD_POOL_SIZE)//原子操作(cmd_now)
			...
			// 等待当前任务完成 //
			if(target->option & amc::plan::Plan::WAIT_FOR_EXECUTION)waitForAllExecution();
		}

		// collect //
		if (!(target->option & amc::plan::Plan::NOT_RUN_COLLECT_FUNCTION))
		{
			// 没有实时规划的轨迹，直接同步收集 //
			if (target->option & amc::plan::Plan::NOT_RUN_EXECUTE_FUNCTION)
			{
				// 等待所有任务完成，原子操作 //
				while ((target->option & amc::plan::Plan::COLLECT_WHEN_ALL_PLAN_EXECUTED) && (cmd_end != imp_->cmd_now_.load()))std::this_thread::sleep_for(std::chrono::milliseconds(1));

				// 等待所有任务收集，原子操作 //
				while ((target->option & amc::plan::Plan::COLLECT_WHEN_ALL_PLAN_COLLECTED) && (cmd_end != imp_->cmd_collect_.load()))std::this_thread::sleep_for(std::chrono::milliseconds(1));

				LOG_INFO << "server collect cmd " << target->command_id << std::endl;
				plan_iter->collectNrt(*target);
				internal_data->target->ret_code = amc::plan::PlanTarget::SUCCESS;
			...
			{
				// 等待当前任务收集 //
				if (target->option & amc::plan::Plan::WAIT_FOR_COLLECTION)waitForAllCollection();
			}
		}
		else
		{
			...
			while (this->imp_->is_collect_running_)
			{
				...
				if (cmd_collect < cmd_now)
				{
					...
					LOG_INFO << "cmd " << target.command_id << " stastics:" << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "avg time(ns):" << std::int64_t(target.rt_stastic.avg_time_consumed) << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "max time(ns):" << target.rt_stastic.max_time_consumed << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "in count:" << target.rt_stastic.max_time_occur_count << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "min time(ns):" << target.rt_stastic.min_time_consumed << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "in count:" << target.rt_stastic.min_time_occur_count << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "total count:" << target.rt_stastic.total_count << std::endl
						<< std::setw(amc::core::LOG_SPACE_WIDTH) << '|' << std::setw(20) << "overruns:" << target.rt_stastic.overrun_count << std::endl;
	...
	{
		// create instance //
		makeModel<amc::dynamic::Model>("model");
		makeController<amc::control::Controller>("controller");
		makeSensorRoot<amc::sensor::SensorRoot>("sensor_root");
		makePlanRoot<amc::plan::PlanRoot>("plan_root");
