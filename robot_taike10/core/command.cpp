...
namespace amc::core
{
	...
	AMC_DEFINE_BIG_FOUR_CPP(ParamBase);
	...
	auto Param::saveXml(amc::core::XmlElement &xml_ele) const->void
	{
	...
	auto Param::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
	...
	auto UniqueParam::saveXml(amc::core::XmlElement &xml_ele) const->void
	{
		ParamBase::saveXml(xml_ele);
		if (!imp_->default_value_.empty())xml_ele.SetAttribute("default", imp_->default_value_.c_str());
	}
	auto UniqueParam::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		ParamBase::loadXml(xml_ele);
		imp_->default_value_ = attributeString(xml_ele, "default", imp_->default_value_);
	}
	...
	AMC_DEFINE_BIG_FOUR_CPP(UniqueParam);
	...
	auto Command::saveXml(amc::core::XmlElement &xml_ele) const->void
	{
		ObjectPool<ParamBase>::saveXml(xml_ele);
		if (!imp_->default_value_.empty())xml_ele.SetAttribute("default", imp_->default_value_.c_str());
	}
	auto Command::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
	...
	AMC_DEFINE_BIG_FOUR_CPP(Command);

	struct CommandParser::Imp { ObjectPool<Command>* command_pool_; };
	auto CommandParser::loadXml(const amc::core::XmlElement &xml_ele)->void
	{
		Object::loadXml(xml_ele);
		imp_->command_pool_ = findOrInsertType<amc::core::ObjectPool<Command>>();
	}
	...
	{
		this->registerType<amc::core::ObjectPool<Command> >();
		imp_->command_pool_ = &add<amc::core::ObjectPool<Command> >("command_pool");
	}
	AMC_DEFINE_BIG_FOUR_CPP(CommandParser);
}
