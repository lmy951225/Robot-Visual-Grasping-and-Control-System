...
namespace amc::control
{
	...
	auto amc_mlockall()->void { if (mlockall(MCL_CURRENT | MCL_FUTURE) == -1) throw std::runtime_error("lock failed"); }
	...
	auto amc_rt_task_start(std::any& rt_task, void(*task_func)(void*), void*param)->int { return rt_task_start(&std::any_cast<RT_TASK&>(rt_task), task_func, param); }
	auto amc_rt_task_join(std::any& rt_task)->int { return rt_task_join(&std::any_cast<RT_TASK&>(rt_task)); }
	auto amc_rt_task_set_periodic(int nanoseconds)->int 
	{ 
		last_time_ = amc_rt_timer_read();
	...
	auto amc_rt_timer_read()->std::int64_t { return rt_timer_read(); }

	auto amc_rt_time_since_last_time()->std::int64_t { return amc_rt_timer_read() - last_time_; }
#else
	...
	auto amc_rt_task_create()->std::any { return std::make_shared<std::thread>(); }
