#ifndef AMC_PLAN_ALGORITHM_H_
#define AMC_PLAN_ALGORITHM_H_
...
//#include <amc/core/core.hpp>
//#include <amc/control/control.hpp>
...
namespace amc::plan
{
	...
	public:
		...
		auto setSolver(amc::dynamic::InverseKinematicSolver *solver)->void { this->solver = solver; }
		auto setModel(amc::dynamic::Model *model)->void { this->model = model; }
		auto run()->void;
	...
	public:
		...
		auto cptDdsConstraint(double s, double ds, double &max_dds, double &min_dds, bool is_forward)->bool
		{
			...
			for (Size i = 0; i < model->generalMotionPool().size(); ++i)
			{
				...
				double real_vs[6];
				amc::dynamic::s_vc(6, ds, vs, real_vs);
			...
			// 根据加速度限制列不等式 //
			auto la = amin;
			amc::dynamic::s_mma(solver->mJi(), 1, solver->nJi(), -ds * ds, solver->Ji(), ddpq_ds2.data(), la.data());
			amc::dynamic::s_vs(solver->mJi(), solver->ci(), la.data());

			auto ra = amax;
			amc::dynamic::s_mma(solver->mJi(), 1, solver->nJi(), -ds * ds, solver->Ji(), ddpq_ds2.data(), ra.data());
			amc::dynamic::s_vs(solver->mJi(), solver->ci(), ra.data());

			std::vector<double> A(solver->mJi());
			amc::dynamic::s_mm(solver->mJi(), 1, solver->nJi(), solver->Ji(), dpq_ds.data(), A.data());

			// 根据速度限制列不等式 //
			auto lv = vmin;
			amc::dynamic::s_va(solver->mJi(), is_forward ? -ds : ds, A.data(), lv.data());

			auto rv = vmax;
			amc::dynamic::s_va(solver->mJi(), is_forward ? -ds : ds, A.data(), rv.data());

			std::vector<double> B(solver->mJi());
			amc::dynamic::s_vc(solver->mJi(), dt, A.data(), B.data());

			// 求解不等式 //
			max_dds = std::numeric_limits<double>::max();
			min_dds = std::numeric_limits<double>::lowest();
			for (amc::Size i = 0; i < solver->mJi(); ++i)
			{
		...
		amc::dynamic::Model *model;
		amc::dynamic::InverseKinematicSolver *solver;
