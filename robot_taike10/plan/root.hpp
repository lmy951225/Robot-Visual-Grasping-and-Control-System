#ifndef AMC_PLAN_ROOT_H_
#define AMC_PLAN_ROOT_H_
...
namespace amc::server { class ControlServer; }
...
namespace amc::plan
{
	...
	{	
		...
		amc::server::ControlServer* server;              // prepair/execute/collect  get&set(but be careful when prepair)
		amc::dynamic::Model* model;                      // prepair/execute/collect  get&set(but be careful when prepair)
		amc::control::Controller* controller;            // prepair/execute/collect  get&set(but be careful when prepair)
		...
		amc::control::Master::RtStasticsData rt_stastic; //                /collect  get
		std::any ret;
		std::int32_t ret_code;
		std::future<void> finished;
	};
	class Plan :public amc::core::Object
	{
	public:
		...
		auto command()->amc::core::Command &;
		auto command()const->const amc::core::Command & { return const_cast<std::decay_t<decltype(*this)> *>(this)->command(); }

		virtual ~Plan();
		explicit Plan(const std::string &name = "plan");
		AMC_REGISTER_TYPE(Plan);
		AMC_DECLARE_BIG_FOUR(Plan);

	private:
		struct Imp;
		amc::core::ImpPtr<Imp> imp_;
	};
	class PlanRoot :public amc::core::Object
	{
	public:
		auto planPool()->amc::core::ObjectPool<Plan> &;
		auto planPool()const->const amc::core::ObjectPool<Plan> & { return const_cast<std::decay_t<decltype(*this)> *>(this)->planPool(); }
		auto planParser()->amc::core::CommandParser;

		virtual ~PlanRoot();
		explicit PlanRoot(const std::string &name = "plan_root");
		AMC_REGISTER_TYPE(PlanRoot);
		AMC_DECLARE_BIG_FOUR(PlanRoot);
	...
	public:
		...
		AMC_REGISTER_TYPE(Enable);
		AMC_DECLARE_BIG_FOUR(Enable);
	...
	public:
		...
		AMC_REGISTER_TYPE(Disable);
		AMC_DECLARE_BIG_FOUR(Disable);
	...
	/// + 按照绝对地址（absID），例如绝对的 0 号电机：“hm -m=0” 或 “hm --motion_id=0”
	/// + 按照物理地址（phyID），例如物理的 2 号电机：“hm -p=2” 或 “hm --physical_id=2”
	/// + 按照从站地址（slaID），例如 5 号从站：“hm -s=5” 或 “hm --slave_id=5”
	/// 
	/// 指定本指令的最长运行时间（默认为5000ms）：
	/// + 回零0号电机，并指定其最长时间为5000ms： “hm -m=0 --limit_time=5000”
	/// 
	...
	public:
		...
		AMC_REGISTER_TYPE(Home);
		AMC_DECLARE_BIG_FOUR(Home);
	...
	public:
		...
		AMC_REGISTER_TYPE(Mode);
		AMC_DECLARE_BIG_FOUR(Mode);

	private:
		struct Imp;
		amc::core::ImpPtr<Imp> imp_;
	};
	/// \brief 电流模式下的关节补偿算法(单关节或者多关节零力拖动)
	/// 
	/// ### 参数定义 ###
	/// 
	/// 补偿指令(默认为位置模式，重力及摩擦力补偿系数为0)：
	/// + 关节一补偿指令： “dg -p=0 --mode=10 --fc1=30 --fv1=50 --limit_time=30000”
	/// + 关节二补偿指令: “dg -p=1 --mode=10 --fc2=20 --fv2=40 --g21=-400 --limit_time=30000”
	/// + 关节三补偿指令: “dg -p=2 --mode=10 --fc3=10 --fv3=40 --g31=200 --g32=70 --limit_time=30000”
	/// + 关节四补偿指令: “dg -p=3 --mode=10 --g4=-40 --limit_time=30000”
	/// + 关节五补偿指令: “dg -p=4 --mode=10 --fc5=20 --fv5=40 --limit_time=30000”
	/// + 五关节补偿指令: “dg -a --mode=10 --fc1=30 --fv1=50 --fc2=20 --fv2=40 --g21=-300 --g22=-100 --g23=-20 --fc3=10 --fv3=40 --g31=200 --g32=70 --fc4=10 --fv4=20 --g4=-40 --fc5=20 --fv5=40 --limit_time=30000”
	class Drag : public Plan
	{
	public:
		auto virtual prepairNrt(const std::map<std::string, std::string> &params, PlanTarget &target)->void override;
		auto virtual executeRT(PlanTarget &target)->int override;
		...
		virtual ~Drag();
		explicit Drag(const std::string &name = "drag_plan");
		AMC_REGISTER_TYPE(Drag);
		AMC_DECLARE_BIG_FOUR(Drag);
	...
	public:
		...
		AMC_REGISTER_TYPE(Reset);
		AMC_DECLARE_BIG_FOUR(Reset);
	...
	public:
		...
		AMC_REGISTER_TYPE(Recover);
		AMC_DECLARE_BIG_FOUR(Recover);
	};
	/// \brief 实时循环内休息指定时间
	/// 
	///
	...
	public:
		...
		AMC_REGISTER_TYPE(Sleep);
		AMC_DECLARE_BIG_FOUR(Sleep);
	...
	public:
		...
		AMC_REGISTER_TYPE(Show);
		AMC_DECLARE_BIG_FOUR(Show);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(MoveAbsJ);
		AMC_DECLARE_BIG_FOUR(MoveAbsJ);
	...
	public:
		...
		AMC_REGISTER_TYPE(MoveJ);
		AMC_DECLARE_BIG_FOUR(MoveJ);
	...
	public:
		...
		AMC_REGISTER_TYPE(MoveL);
		AMC_DECLARE_BIG_FOUR(MoveL);
	...
	/// \brief 关节空间轨迹规划，采用T型插补算法,对插补点位置，速度，加减速度进行规划并求出轨迹
	/// 
	/// ### 参数定义 ###
	///
	/// 指定初始位姿与目标位姿，可以使用以下参数：
	/// + 位置与欧拉角，位置姿态：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0}”
	/// 
	/// 此外，还可以指定位置和角度的单位，长度单位默认为 m ，角度默认为 rad ：
	/// + 指定角度单位，例如将单位设置为 rad ：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pos_unit=m --ori_unit=rad”
	///
	/// 还可以指定欧拉角的种类，可以是 321 313 123 212 ... 等任意类型的欧拉角，默认为 321 的欧拉角
	/// + 指定欧拉角为 123 的，“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --ori_unit=rad --eul_type=321”
	///
	/// 指定关节速度，单位一般是 m/s 或 rad/s ，应永远为正数，默认为0.1
	/// + 指定所有电机的速度都为0.5：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5”
	/// + 指定所有电机的速度：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel={0.2,0.2,0.2,0.3,0.3,0.3}”
	///
	/// 指定关节加速度，单位一般是 m/s^2 或 rad/s^2 ，应永远为正数，默认为0.1
	/// + 指定所有电机的加速度都为0.3：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5 --joint_acc=0.3”
	/// + 指定所有电机的加速度：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5 --joint_acc={0.2,0.2,0.2,0.3,0.3,0.3}”
	class PtpJ : public Plan
	{
	public:
		auto virtual prepairNrt(const std::map<std::string, std::string> &params, PlanTarget &target)->void override;
		auto virtual executeRT(PlanTarget &target)->int override;
		...
		virtual ~PtpJ();
		explicit PtpJ(const std::string &name = "ptp_j");
		AMC_REGISTER_TYPE(PtpJ);
		AMC_DECLARE_BIG_FOUR(PtpJ);
	...
	/// \brief 笛卡尔空间直线轨迹规划，先线性插补，再对插补点位置，速度，加减速度进行规划并求出轨迹
	/// 
	/// 
	/// ### 参数定义 ###
	///
	/// 指定初始位姿与目标位姿，可以使用以下参数：
	/// + 位置与欧拉角：“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0}”
	///
	/// 此外，还可以指定位置和角度的单位，长度单位默认为 m ，角度默认为 rad ：
	/// + 指定角度单位，例如将单位设置为 rad ：“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pos_unit=m --ori_unit=rad”
	///
	/// 还可以指定欧拉角的种类，可以是 321 313 123 212 ... 等任意类型的欧拉角，默认为 321 的欧拉角
	/// + 指定欧拉角为 123 的，“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --ori_unit=rad --eul_type=321”
	///
	/// 指定关节速度，单位一般是 m/s 或 rad/s ，应永远为正数，默认为0.1
	/// + 指定所有电机的速度都为0.5：“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5”
	/// + 指定所有电机的速度：“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel={0.2,0.2,0.2,0.3,0.3,0.3}”
	///
	/// 指定关节加速度，单位一般是 m/s^2 或 rad/s^2 ，应永远为正数，默认为0.1
	/// + 指定所有电机的加速度都为0.3：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5 --joint_acc=0.3”
	/// + 指定所有电机的加速度：“ptpl --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5 --joint_acc={0.2,0.2,0.2,0.3,0.3,0.3}”
	class PtpL : public Plan
	{
	public:
		auto virtual prepairNrt(const std::map<std::string, std::string> &params, PlanTarget &target)->void override;
		auto virtual executeRT(PlanTarget &target)->int override;
		...
		virtual ~PtpL();
		explicit PtpL(const std::string &name = "ptp_l");
		AMC_REGISTER_TYPE(PtpL);
		AMC_DECLARE_BIG_FOUR(PtpL);
	...
	/// \brief 笛卡尔空间圆弧轨迹规划，先圆弧插补，再对插补点位置，速度，加减速度进行规划并求出轨迹
	/// 
	/// 
	/// ### 参数定义 ###
	///
	/// 指定初始位姿与目标位姿，可以使用以下参数：
	/// + 位置与欧拉角：“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0}”
	///
	/// 此外，还可以指定位置和角度的单位，长度单位默认为 m ，角度默认为 rad ：
	/// + 指定角度单位，例如将单位设置为 rad ：“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0} --pos_unit=m --ori_unit=rad”
	///
	/// 还可以指定欧拉角的种类，可以是 321 313 123 212 ... 等任意类型的欧拉角，默认为 321 的欧拉角
	/// + 指定欧拉角为 123 的，“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0} --ori_unit=rad --eul_type=321”
	///
	/// 指定关节速度，单位一般是 m/s 或 rad/s ，应永远为正数，默认为0.1
	/// + 指定所有电机的速度都为0.5：“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0} --joint_vel=0.5”
	/// + 指定所有电机的速度：“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0} --joint_vel={0.2,0.2,0.2,0.3,0.3,0.3}”
	///
	/// 指定关节加速度，单位一般是 m/s^2 或 rad/s^2 ，应永远为正数，默认为0.1
	/// + 指定所有电机的加速度都为0.3：“ptpj --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --joint_vel=0.5 --joint_acc=0.3”
	/// + 指定所有电机的加速度：“ptpr --pe1={0,0.5,1.1,0,0,0} --pe2={0.2,0.6,0.8,0,0,0} --pe3={0.4,0.8,0.5,0,0,0} --joint_vel=0.5 --joint_acc={0.2,0.2,0.2,0.3,0.3,0.3}”
	class PtpR : public Plan
	{
	public:
		auto virtual prepairNrt(const std::map<std::string, std::string> &params, PlanTarget &target)->void override;
		auto virtual executeRT(PlanTarget &target)->int override;
		...
		virtual ~PtpR();
		explicit PtpR(const std::string &name = "ptp_r");
		AMC_REGISTER_TYPE(PtpR);
		AMC_DECLARE_BIG_FOUR(PtpR);
	...
	public:
		...
		AMC_REGISTER_TYPE(AutoMove);
		AMC_DECLARE_BIG_FOUR(AutoMove);
	...
	public:
		...
		AMC_REGISTER_TYPE(ManualMove);
		AMC_DECLARE_BIG_FOUR(ManualMove);
	...
	public:
		...
		AMC_REGISTER_TYPE(GetPartPq);
		AMC_DEFINE_BIG_FOUR(GetPartPq);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(GetXml);
		AMC_DEFINE_BIG_FOUR(GetXml);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(SetXml);
		AMC_DEFINE_BIG_FOUR(SetXml);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(Start);
		AMC_DEFINE_BIG_FOUR(Start);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(Stop);
		AMC_DEFINE_BIG_FOUR(Stop);
	};

	class RemoveFile : public amc::plan::Plan
	{
	public:
		auto virtual prepairNrt(const std::map<std::string, std::string> &params, amc::plan::PlanTarget &target)->void;
		
		virtual ~RemoveFile();
		explicit RemoveFile(const std::string &name = "rm_file");
		AMC_REGISTER_TYPE(RemoveFile);
		AMC_DEFINE_BIG_FOUR(RemoveFile);
	};
	...
	public:
		...
		AMC_REGISTER_TYPE(UniversalPlan);
		AMC_DECLARE_BIG_FOUR(UniversalPlan);
