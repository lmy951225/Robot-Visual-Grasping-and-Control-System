...
namespace amc::robot
{
	...
	auto createModelRokaeXB4(const double *robot_pm)->std::unique_ptr<amc::dynamic::Model> 
	{ 
		amc::dynamic::PumaParam param;
		param.d1 = 0.3295;
		param.a1 = 0.04;
		param.a2 = 0.275;
		param.d3 = 0.0;
		...
		return amc::dynamic::createModelPuma(param);
	}
	auto createPlanRootRokaeXB4()->std::unique_ptr<amc::plan::PlanRoot>
	{
		...
		plan_root->planPool().add<amc::plan::Disable>();
		plan_root->planPool().add<amc::plan::Home>();
		plan_root->planPool().add<amc::plan::Mode>();
		plan_root->planPool().add<amc::plan::Show>();
		plan_root->planPool().add<amc::plan::Sleep>();
		plan_root->planPool().add<amc::plan::Recover>();
		auto &rs = plan_root->planPool().add<amc::plan::Reset>();
		rs.command().findParam("pos")->setDefaultValue("{0.5,0.392523364485981,0.789915966386555,0.5,0.5,0.5}");

		auto &mvaj = plan_root->planPool().add<amc::plan::MoveAbsJ>();
		mvaj.command().findParam("vel")->setDefaultValue("0.1");

		plan_root->planPool().add<amc::plan::MoveL>();
		plan_root->planPool().add<amc::plan::MoveJ>();

		plan_root->planPool().add<amc::plan::GetPartPq>();
		plan_root->planPool().add<amc::plan::GetXml>();
		plan_root->planPool().add<amc::plan::SetXml>();
		plan_root->planPool().add<amc::plan::Start>();
		plan_root->planPool().add<amc::plan::Stop>();
