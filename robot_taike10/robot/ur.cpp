...
{
	// 具体参数参考MR里面147-158页 //
	auto createModelUr5(const double *robot_pm)->std::unique_ptr<amc::dynamic::Model>
	{
		...
		// add solid //
		model->ground().geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/ground.DWG")
			, s_pe2pm(std::array<double, 6>{0, 0, 0.003, 2.3561944875, -PI / 2, 0.0 }.data(), nullptr, "313"));
		p1.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L1.DWG")
			, s_pe2pm(std::array<double, 6>{0.0, 0.0, 0.089159, 0, -PI / 2, 0}.data(), nullptr, "313"));
		p2.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L2.DWG")
			, s_pe2pm(std::array<double, 6>{ 0.0, 0.13585, 0.089159, 0, -PI / 2, PI / 2}.data(), nullptr, "313"));
		p3.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L3.DWG")
			, s_pe2pm(std::array<double, 6>{0.425, 0.13585 - 0.1197, 0.089159, 0, -PI / 2, PI / 2}.data(), nullptr, "313"));
		p4.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L4.DWG")
			, s_pe2pm(std::array<double, 6>{0.425 + 0.39225, 0.13585 - 0.1197 + 0.093, 0.089159, 0, -PI / 2, -PI / 2}.data(), nullptr, "321"));
		p5.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L5.DWG")
			, s_pe2pm(std::array<double, 6>{0.425 + 0.39225, 0.13585 - 0.1197 + 0.093, 0.089159 - 0.09465, 0, -PI / 2, PI}.data(), nullptr, "313"));
		p6.geometryPool().add<amc::dynamic::FileGeometry>("file_geometry", AMC_INSTALL_PATH + std::string("/resource/amc_robot/ur5/L6.DWG")
		...
		// add solver
		//auto &inverse_kinematic = model->solverPool().add<amc::dynamic::Ur5InverseKinematicSolver>();
		auto &inverse_kinematic = model->solverPool().add<amc::dynamic::InverseKinematicSolver>();
	...
	{
		std::unique_ptr<amc::control::Controller> controller(new amc::control::EthercatController);
		...
		for (amc::Size i = 0; i < 6; ++i)
		{
			...
			controller->slavePool().add<amc::control::EthercatMotion>().loadXmlStr(xml_str);
		}
	...
	{
		std::unique_ptr<amc::plan::PlanRoot> plan_root(new amc::plan::PlanRoot);

		plan_root->planPool().add<amc::plan::Enable>();
		plan_root->planPool().add<amc::plan::MoveL>();
		plan_root->planPool().add<amc::plan::MoveJ>();
